...
def train_tune_test(ds, train_size=.90, tune_size=.1, test_size=0., withhold=None,
                    rseed=8, out_dir=None, overwrite=False, n_mut_split=True):
    ...
    if n_mut_split:
        single_idxs = np.where(ds["num_mutations"] == 1)
        split["single"] = single_idxs
        double_idxs = np.where(ds["num_mutations"] == 2)
        split["double"] = double_idxs
...
def cross_validation_split(ds, withhold=None, train_size=.90, tune_size=.1, test_size=0.,
                    method="kfold", rseed=8, n_splits=10, out_dir=None, overwrite=False):
    ...
    if test_size > 0:
        if test_size == 1:
            ...
            idxs, test_idxs = train_test_split(idxs, test_size=test_size)
    
    adjusted_tune_size = np.around(tune_size / (1 - test_size), 5)    
    adjusted_train_size = np.around(train_size / (1 - tune_size - test_size), 5)
    
    if method == "KFold":
    
        kf = KFold(n_splits=n_splits)
        for i, (train_idxs, tune_idxs) in enumerate(kf.split(idxs)):
            if test_size > 0:
                split[f"Fold_{i}:"] = {"train": train_idxs, "tune": tune_idxs, "test": test_idxs}
            else:    
                split[f"Fold_{i}:"] = {"train": train_idxs, "tune": tune_idxs}
            
    elif method == "ShuffleSplit":
        
        ss = ShuffleSplit(n_splits=n_splits, test_size=adjusted_tune_size, random_state=rseed)
        for i, (train_idxs, tune_idxs) in enumerate(ss.split(idxs)):
            if test_size > 0:
                split[f"Fold_{i}:"] = {"train": train_idxs, "tune": tune_idxs, "test": test_idxs}
            else:    
                split[f"Fold_{i}:"] = {"train": train_idxs, "tune": tune_idxs}
